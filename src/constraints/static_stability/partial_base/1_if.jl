...
function if_pb(m)
    ...
    #Xj < Xi + Xcoveri
    @variable(m, if2_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], Bin)
    @constraint(m, c1_if2_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        L * (1 - m[:if2_pbs][i, j]) >= 
        m[:flbx_front_left_bot_x][j] + 0.001 - (m[:flbx_front_left_bot_x][i] + m[:box_x_axis_cover][i])
    )

    @constraint(m, c2_if2_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        L * m[:if2_pbs][i, j] >= 
        (m[:flbx_front_left_bot_x][i] + m[:box_x_axis_cover][i]) - m[:flbx_front_left_bot_x][j] - 0.001
    )

    # Xj + Xcoverj > Xi
    @variable(m, if3_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], Bin)
    @constraint(m, c1_if3_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        L * (1 - m[:if3_pbs][i, j]) >= 
        m[:flbx_front_left_bot_x][i] + 0.001 - (m[:flbx_front_left_bot_x][j] + m[:box_x_axis_cover][j])
    )

    @constraint(m, c2_if3_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        L * m[:if3_pbs][i, j] >= 
        (m[:flbx_front_left_bot_x][j] + m[:box_x_axis_cover][j]) - m[:flbx_front_left_bot_x][i] - 0.001
    )

    # Yj < Yi + Ycoveri
    @variable(m, if4_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], Bin)
    @constraint(m, c1_if4_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        W * (1 - m[:if4_pbs][i, j]) >= 
        m[:flby_front_left_bot_y][j] + 0.001 - (m[:flby_front_left_bot_y][i] + m[:box_y_axis_cover][i])
    )

    @constraint(m, c2_if4_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        W * m[:if4_pbs][i, j] >= 
        (m[:flby_front_left_bot_y][i] + m[:box_y_axis_cover][i]) - m[:flby_front_left_bot_y][j] - 0.001
    )

    # Yj + Ycoverj > Yi
    @variable(m, if5_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], Bin)
    @constraint(m, c1_if5_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        W * (1 - m[:if5_pbs][i, j]) >= 
        m[:flby_front_left_bot_y][i] + 0.001 - (m[:flby_front_left_bot_y][j] + m[:box_y_axis_cover][j])
    )

    @constraint(m, c2_if5_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        W * m[:if5_pbs][i, j] >= 
    ...
    @constraint(m, c_if_pbs[i=1:length(boxes), j=1:length(boxes); i!=j], 
        m[:if1_pbs][i, j] + 
        m[:if2_pbs][i, j] + 
        m[:if3_pbs][i, j] +
        m[:if4_pbs][i, j] + 
